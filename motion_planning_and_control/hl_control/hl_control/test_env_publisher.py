#!/usr/bin/env python3
"""
TC2.1 test environment publisher.

Reads a board YAML and publishes once to:
  /perception/gameboard_state  (hl_control/GameboardState) — latched
  /hl_control/word_request     (std_msgs/String)

Usage (via launch file):
  ros2 run hl_control test_env_publisher.py
    --ros-args -p config_path:=<path/to/board.yaml> -p word:=CRANE
